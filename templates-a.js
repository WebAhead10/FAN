...
function newPost() {
  return layout(/*html */ `
    ...
    <form action="/adan-newPost" method="POST">
...
function allPosts(posts) {
  return layout(/*html */ `
    ...
    <ul>
      ${posts
        .map(
          (post) => `
          <li>
            <a class="a" href="/adan-posts/${post.title}">${post.title}</a>
   ...
   <a href="/profiles/adan"> <button class="btn1" > Back </button></a>
  `);
}
